...
componentDidMount() {
  ...
  console.log("friends uid: " + friendUid)
  db.collection("Users")
   ...
   .doc(friendUid).collection("Library")
      ...
      if(this.state.bookcount > 0){
        ...
        return(
          <div className = 'library'>
            ...
            <Avatar className = "ava" name={xxx} round={true}/>
              <Card.Title>{xxx} is reading</Card.Title>
    ...
    return(
      <div className = 'library'>
        <CardColumns className = 'post'>
        <Card className = 'currentRead'>
        <Avatar style = {{marginBottom: '30px'}} className = "ava" name={xxx} round={true}/>
          <Card.Title>{xxx} is not reading anything!</Card.Title>
        </Card>
    {this.state.data.map((data, index) => {
     return(
       <div>
